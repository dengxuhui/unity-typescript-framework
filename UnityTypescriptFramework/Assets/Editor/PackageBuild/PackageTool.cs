...
public class PackageTool : EditorWindow
{
    ...
    static private string buildVersion = "0";
    ...
    static private BuildSetting buildSetting;
    ...
    public static void SaveAllCurrentVersionFile(bool silence = false)
    {
        ...
        int.TryParse(buildVersion, out buildVersionCode);
    ...
    static void InitBuildSetting()
    {
        ...
        foreach (string str in parameters)
        {
            ...
            else if (str.StartsWith("BuildVersion"))
            {
                ...
                if (tempParam.Length == 2)
                {
                    buildSetting.BuildVersion = tempParam[1].Trim();
                }
            }
            else if (str.StartsWith("ResVersion"))
            {
                ...
                if (tempParam.Length == 2)
                {
                    buildSetting.ResVersion = "1.0." + tempParam[1].Trim();
                }
            }
            else if (str.StartsWith("KeystoreName"))
            {
                var tempParam = str.Split(new string[] { "=" }, StringSplitOptions.RemoveEmptyEntries);
                if (tempParam.Length == 2)
                {
                    buildSetting.KeystoreName = tempParam[1].Trim();
                }
            }
            else if (str.StartsWith("KeystorePass"))
            {
                var tempParam = str.Split(new string[] { "=" }, StringSplitOptions.RemoveEmptyEntries);
                if (tempParam.Length == 2)
                {
                    buildSetting.KeystorePass = tempParam[1].Trim();
                }
            }
            else if (str.StartsWith("KeyaliasName"))
            {
                var tempParam = str.Split(new string[] { "=" }, StringSplitOptions.RemoveEmptyEntries);
                if (tempParam.Length == 2)
                {
                    buildSetting.KeyaliasName = tempParam[1].Trim();
                }
            }
            else if (str.StartsWith("KeyaliasPass"))
            {
                var tempParam = str.Split(new string[] { "=" }, StringSplitOptions.RemoveEmptyEntries);
                if (tempParam.Length == 2)
                {
                    buildSetting.KeyaliasPass = tempParam[1].Trim();
                }
            }
            else if (str.StartsWith("BuildAppBundle"))
            {
                var tempParam = str.Split(new string[] { "=" }, StringSplitOptions.RemoveEmptyEntries);
                if (tempParam.Length == 2)
                {
                    buildSetting.BuildAppBundle = tempParam[1].Trim() == "true";
                }
            }
            else if (str.StartsWith("Development"))
            {
                var tempParam = str.Split(new string[] { "=" }, StringSplitOptions.RemoveEmptyEntries);
                if (tempParam.Length == 2)
                {
                    buildSetting.Development = tempParam[1].Trim() == "true";
                }
            }
            else if (str.StartsWith("Architecture"))
            {
                ...
                if (tempParam.Length == 2)
                {
                    int.TryParse(tempParam[1].Trim(), out buildSetting.Architecture);
                }
            }
        }
    ...
    public static void BuildAssetBundles()
    {
        ...
        resVersion = buildSetting.ResVersion;
        bundleVersion = buildSetting.Version;
        buildVersion = buildSetting.BuildVersion;
        SaveAllCurrentVersionFile(true);
        BuildPlayer.BuildAssetBundles(buildTarget, channelType.ToString());
    }

    /// <summary>
    /// 拷贝资源到StreamingAssets
    /// </summary>
    ...
    public static void BuildAndroid()
    {
        ...
        PlayerSettings.keystorePass = buildSetting.KeystorePass;
        PlayerSettings.Android.keyaliasName = buildSetting.KeyaliasName;
        PlayerSettings.keyaliasPass = buildSetting.KeyaliasPass;
        // 不显示SplashScreen
        ...
        // 库
        if (buildSetting.BuildAppBundle)
        {
            PlayerSettings.Android.targetArchitectures = AndroidArchitecture.ARMv7 | AndroidArchitecture.ARM64;
        }
        else
        {
            PlayerSettings.Android.targetArchitectures = (AndroidArchitecture)buildSetting.Architecture;
        }
        // aab或者apk
        EditorUserBuildSettings.buildAppBundle = buildSetting.BuildAppBundle;
        EditorUserBuildSettings.development = buildSetting.Development;
        if (!buildSetting.Development)
        {
            var symbols = "HOTFIX_ENABLE";
            PlayerSettings.SetScriptingDefineSymbolsForGroup(BuildTargetGroup.Android, symbols);
        }
        BuildPlayer.BuildAndroid(channelType.ToString(), false);
    }

...
export class LanguageManager extends EventDispatcher implements ISingleton {
    ...
    public getStringByFullKey(key: string): string {
        ...
        if (string.IsNullOrEmpty(value)) {
            CS.Logger.LogError(`LanguageManager::获取字符串错误,key->${key}`);
            return string.empty;
        } else {
            return value;
        }
    ...
    private _parseCSV(asset: UnityEngine.TextAsset) {
        if (asset == null) {
            CS.Logger.LogError("update language fail,text asset is null");
            return;
        }
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (kv.length != 2) {
                CS.Logger.LogError(`language asset parse error,len not equal 2,kv:${array[i]}`);
                continue;
            }
